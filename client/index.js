...
app.post('/api/pos/waiter-settings', async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, locationId: loc, waiterEnabled: eff.enabled, hasWaiterPin: !!eff.pin, waiterTables: eff.tables, waiterPinMode: eff.mode, waiterStaff: eff.staff.map((s) => ({ id: s.id, name: s.name })) });
  } catch (e) { res.status(400).json({ error: e.message }); }
});
