...
export const api = {
  ...
  getMenu: (location, pos) => req(`/api/menu?${location ? `location=${encodeURIComponent(location)}` : ''}${pos ? `${location ? '&' : ''}pos=1` : ''}`),
  ...
  adminBirthdays: (pass) => req(`/api/admin/birthdays?pass=${encodeURIComponent(pass || '')}`),
  adminSetBirthday: (pass, customerId, birthday) => req(`/api/admin/birthday/set?pass=${encodeURIComponent(pass || '')}`, { method: 'POST', body: JSON.stringify({ customerId, birthday }) }),
  ...
  loyaltyProfile: (pass, customerId, patch) => req(`/api/admin/loyalty/profile?pass=${encodeURIComponent(pass || '')}`, { method: 'POST', body: JSON.stringify({ customerId, ...patch }) }),
  ...
  posTerminalPair: (pass, name, locationId) => req(`/api/pos/terminal/pair?pass=${encodeURIComponent(pass || '')}`, { method: 'POST', body: JSON.stringify({ name, locationId }) }),
  posTerminalRemove: (pass, deviceId) => req(`/api/pos/terminal/remove?pass=${encodeURIComponent(pass || '')}`, { method: 'POST', body: JSON.stringify({ deviceId }) }),
